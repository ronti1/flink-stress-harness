...
Wired in `HarnessJob.buildPipeline`. Watermarks are only added for event-time
windows (TUMBLING_TIME / SESSION). Fault stages are inserted only when a fault
targets that stage.
...
| `fault/` | `FaultInjectionMap` (generic, pure `evaluate`), `HaltStrategy`/`JvmHaltStrategy`, `FaultConfig` |
...
```powershell
# Build + 59 unit tests + 9 MiniCluster integration tests + JaCoCo coverage
...
- **Testability seams are deliberate:** fault firing is a pure static
  `FaultInjectionMap.evaluate(...)`; the TM-kill is behind `HaltStrategy` so
